                                ...
                                <div class="d-flex justify-content-center">
                                    @if ($isClosed || $isRefused)
                                        ...
                                        <span class="text-success small fw-bold"><i class="bi bi-check-circle-fill"></i>
                                            ปิดงานเรียบร้อย</span>
                                    @elseif ($isRepairFinished)
                                        <button type="button"
                                            class="btn btn-info btn-sm px-3 text-white fw-bold btn-receive-back"
                                        ...
                                        <form id="form-receive-{{ $item->NotirepairId }}"
                                            action="{{ route('noti.close', $item->NotirepairId) }}" method="POST"
                                            style="display: none;">
                                            @csrf
                                        </form>
                                    @else
    ...
    <div class="d-md-none">
        @foreach ($noti as $item)
            ...
            <div class="card mb-3 border-0 shadow-sm"
                style="border-radius: 15px; overflow: hidden; background-color: #fff;">
                <div class="d-flex">
                    ...
                    <div class="card-body p-3">
                        <div class="d-flex justify-content-between align-items-start mb-2 gap-2">
                            ...
                            <div style="min-width: 0; flex: 1;">
                                {{-- เพิ่ม Badge ลำดับตรงนี้ --}}
                                <span class="badge bg-light text-dark border mb-1" style="font-size: 0.65rem;">
                                    ลำดับที่ {{ ($noti->currentPage() - 1) * $noti->perPage() + $loop->iteration }}
                                </span>
                                <br>
                                <span class="text-muted small fw-bold">#{{ $item->JobId ?? $item->NotirepairId }}</span>
                                <h6 class="mb-0 fw-bold text-dark text-truncate">{{ $item->equipmentName }}</h6>
                            ...
                            <span class="badge rounded-pill flex-shrink-0"
                                style="background-color: {{ $themeColor }}; font-size: 0.7rem; padding: 0.4em 0.8em;">
                        ...
                        <div class="d-flex justify-content-between align-items-center mt-auto">
                            <div class="text-muted" style="font-size: 0.75rem;">
                                <i
                                    class="bi bi-calendar3 me-1"></i>{{ date('d/m/y H:i', strtotime($item->DateNotirepair)) }}
                            </div>
        ...
        $(document).ready(function() {
            if (window.matchMedia('(min-width: 768px)').matches) {
                $('#storeTable').DataTable({
                    "searching": false,
                    "paging": false,
                    "ordering": true, // เปิดการเรียงลำดับสำหรับตาราง
                    "info": false,
                    "autoWidth": false,
                    "order": [], // ปล่อยให้เรียงตามที่ฐานข้อมูลส่งมาในตอนแรก
                    "columnDefs": [
                        { 
                            "targets": "_all", 
                            "orderable": true // บังคับเปิด Sorting ให้ทุกคอลัมน์ (Index 0-7)
                        },
                        { 
                            "targets": [0, 1, 2, 4, 5, 6, 7], 
                            "className": "text-center" // จัดกึ่งกลาง ยกเว้นรายละเอียด
                        },
                        { 
                            "targets": 3, 
                            "className": "text-start" // คอลัมน์รายละเอียดให้ชิดซ้าย
                        }
                    ],
                    "language": {
